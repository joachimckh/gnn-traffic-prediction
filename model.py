...
class ASTGCN_Recent(nn.Module):
    def __init__(self,device, num_nodes, dropout=0.3, supports=None,length=12, 
        ...
        ### added ####
        self.block3=ST_BLOCK_0(dilation_channels,dilation_channels,num_nodes,length,K,Kt)
        ##############
...
class LSTM(nn.Module):
    ...
    def forward(self,input):
        ...
        h = Variable(torch.zeros((1,shape[0]*shape[2],self.c_out)))
        c = Variable(torch.zeros((1,shape[0]*shape[2],self.c_out)))
        hidden=(h,c)
